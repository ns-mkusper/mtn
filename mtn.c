...
//                av_log(NULL, LOG_INFO, "   DURATION: %.2f\n", duration);
        ...
        double net_duration;
        if (parameters.gb_x_duration > 0) {
                net_duration=parameters.gb_x_duration;
        } else if (parameters.gb_C_cut > 0) {
...
/* //                                eff_target=MAX(seek_target, eff_target); */
/*                                 if (eff_target > duration_tb || eff_target > parameters.gb_E_end) { // end of file */
/*                         eff_target=(duration_tb - random_at_most(seek_target_step)); */
/*                         av_log(NULL, LOG_INFO, "GOT EOF 1 (DURATION: %.2f EFF_TARGET: %.2f)\n", duration_tb, (eff_target * av_q2d(pStream->time_base))); */
/*                         eof_one_count++; */
/* if (eof_one_count > 6) goto eof; */
/*                 } */
...
//                        av_log(NULL, LOG_INFO, "%s : %d\n", idx, individual_filename, average_color);
                ...
                // PTS STAMP ON FILE
                /* sprintf(individual_suffix, "_%013"PRId64".jpg", found_pts); // */
                /* asprintf(&previous_suffix, "_%013"PRId64".jpg", found_pts); // */
                // SECONDS STAMP ON FILE
...
//                        if (found_pts - prevjpegshot_pts >= seek_target_step) {
                ...
                /*         seek_target_step=(seek_target_step_original); */
                /*         seek_target=(seek_target_step * shots_saved); */
                /* } */
